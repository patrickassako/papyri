...
export default function ProfilePage() {
  ...
  useEffect(() => {
    async function loadProfile() {
      try {
        const [userRes, historyRes, subscriptionRes] = await Promise.allSettled([
          ...
          <Grid size={{ xs: 12, md: 8 }}>
            ...
            <Grid container spacing={2} sx={{ mb: 3 }}>
              {[
                ...
                { icon: HeadphonesOutlined, value: `${stats.totalHours}h`, label: 'TEMPS TOTAL', color: '#2E4057' },
              ].map((stat) => (
      ...
      <Dialog open={avatarDialogOpen} onClose={() => setAvatarDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Box sx={{ mt: 1, mb: 2, display: 'flex', alignItems: 'center', gap: 2 }}>
            <Avatar src={avatarPreview || user?.avatar_url} sx={{ width: 84, height: 84, bgcolor: primary }}>
              {user?.full_name?.[0]?.toUpperCase() || 'U'}
            </Avatar>
            <Box>
        ...
        <DialogContent>
            ...
            InputProps={{
              endAdornment: (
                <InputAdornment position="end">
                  <IconButton onClick={() => setShowPasswords((prev) => ({ ...prev, current: !prev.current }))} edge="end">
          ...
          <TextField
            ...
            InputProps={{
              endAdornment: (
                <InputAdornment position="end">
                  <IconButton onClick={() => setShowPasswords((prev) => ({ ...prev, next: !prev.next }))} edge="end">
          ...
          <TextField
            ...
            InputProps={{
              endAdornment: (
                <InputAdornment position="end">
                  <IconButton onClick={() => setShowPasswords((prev) => ({ ...prev, confirm: !prev.confirm }))} edge="end">
